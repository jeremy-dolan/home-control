...
[midea]
# Pin specific units by IP (skips LAN broadcast discovery for these). Each
# entry's "name" overrides the unit's own reported firmware name (e.g.
# "net_ac_16A4"). Leave unset to auto-discover all reachable units.
# units = [
#   { ip = "192.168.1.50", name = "Living Room" },
# ]
#
# V3 units need a one-time cloud pairing to fetch a token/key (cached locally
# afterward, see ~/.cache/home-control/midea_tokens.json). By default this
# uses msmart-ng's shared demo NetHome Plus account, which can get rate-
# limited if many users hit it at once. Set your own account to avoid that
# (the units must already be added to it in whichever app you use):
...
#   "smarthome"              — the "SmartHome"/MSmartHome app
# cloud = "smarthome"
"""
