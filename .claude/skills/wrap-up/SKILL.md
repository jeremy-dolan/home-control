---
name: wrap-up
description: End-of-session check on whether anything learned this session belongs in ARCHITECTURE.md, AGENTS.md, or memory. High bar — the default and most common outcome is "nothing to do".
---
...
`AGENTS.md`) or your persistent memory.
...
- **Stale** — the work made an existing statement in `ARCHITECTURE.md`,
  `AGENTS.md`, or a memory *wrong* (e.g. a `System` contract method renamed, a
  file moved, config handling changed, a documented gotcha now fixed). This is
  the highest-priority trigger: a wrong doc is worse than a missing one.
- **New, durable, and important** — the session established something future
  ...
  and is not already written down.
...
- **Non-obvious behavior of code that exists** → an inline comment at the site,
  not a doc.
- **Future work / design notes / research** → `TODO.md`.
